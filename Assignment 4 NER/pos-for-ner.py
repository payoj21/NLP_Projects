                ...
                if tag not in tag_dictionary:
                    ...
                    if(count != 0):
                        if(count < len(sentence)-1):
                            tag_dictionary.update({tag:{'Total_count':1,'Count_in_2nd_place':0, 'Count_starting_sentence':0, 'Count_in_1st_place':0, 'Count_ending_sentence': 0}})
                        else:
                            tag_dictionary.update({tag: {'Total_count':1,'Count_in_2nd_place':0, 'Count_starting_sentence':0, 'Count_in_1st_place': 0, 'Count_ending_sentence': 1}})

                    else:
                        tag_dictionary.update({tag:{'Total_count':1, 'Count_in_2nd_place': 0,'Count_starting_sentence': 1,'Count_in_1st_place': 0,'Count_ending_sentence': 0}})
                else:
...
def Laplace(tag_dictionary, bigrams_types):
    ...
    for bigram in bigram_permutation:
        ...
        tag_i_1 = bigram[0]
        if (bigram in bigrams_types):
            transition_probability = float((bigrams_types[bigram]+1)/(tag_dictionary[tag_i_1]['Total_count']+len(tag_dictionary)))
        else:
...
fd = open('jain-payoj-assgn2-test-output.txt','w')
#fd = open('dev_set_output.txt','w')

for sentence in test_data:
#for sentence in dev_set:
    part_of_speech_tagging = viterbi(sentence,word_dictionary,tag_dictionary)
    for index in range(len(sentence)):
        fd.write(str(index+1)+'\t'+sentence[index]+'\t'+part_of_speech_tagging[index]+'\n')
    fd.write('\n')
